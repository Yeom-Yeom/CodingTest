...
class Url implements Comparable<Url>{
    ...
    // 우선순위가 높은 url을 결정하기 위해 정렬함수를 만들어준다.
    @Override
...
public class CodeTree_Grader {
    ...
    public static void init(Scanner sc){
        ...
        // 만약 도메인이 처음 나온 도메인이라면 domainToIdx에 갱신한다.
        ...
        // 도메인 번호에 맞는 레디큐에 숫자 부분을 넣어준다.
        isInReadyq [domainIdx].add(num);

        // 새로 들어온 url을 도메인에 맞춰 urlPq에 넣어준다.
        Url newUrl = new Url(0,1,num);
        urlPq[domainIdx].add(newUrl);

        // 채점 대기 큐에 값이 추가됐으므로 개수를 1 추가한다.
        ans++;
    }

    // 새로운 url을 입력받아 레디큐에 추가해준다.
    public static void newUrl(Scanner sc){
        ...
        // 만약 숫자 부분이 이미 레디큐에 있으면 중복되므로 넘어간다.
    ...
    public static void assign(Scanner sc){
        ...
        for(int i=1; i<=cnt; i++){
            // 만약 현재 채점중이거나, 현재 시간에 이용할 수 없다면 넘어간다.
            if(e[i] > tme) continue;

            // 만약 i번 도메인에 해당하는 url이 존재한다면
            // 해당 도메인에서 가장 우선순위가 높은 url을 뽑고 갱신해준다.
            if(!urlPq[i].isEmpty()){
        ...
        // 만약 가장 우선순이가 높은 url이 존재하면
        // 해당 도메인과 쉬고 있는 가장 낮은 번호의 채점기를 연결
        if(minDomain > 0){
            ...
            // judgerIdx번 채점기가 채점하고 있는 도메인 번호를 갱신해준다.
            judgingDomain[judgerIdx] = minDomain;

            // 레디큐에서 해당 url의 숫자를 지워준다.
            isInReadyq[minDomain].remove(minUrl.num);

            // 채점 대기 큐에 값이 지워졌으므로 개수를 1 감소한다.
            ans--;
        }
    }
